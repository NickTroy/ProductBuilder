class ProductInfoController < ApplicationController
  ...
  def show
    ...
    @variant_images_info = ActiveRecord::Base.connection.execute('select vi.id, vi.image_file_name, v.id from variant_images vi inner join variants v on v.id = vi.variant_id;')
