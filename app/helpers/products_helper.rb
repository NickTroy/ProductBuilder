module ProductsHelper
  ...
  def persist_product data, info, variants
    ...
    if _product.nil?
      ...
      _product.attributes[:tags] = "product"
      byebug
      _product.attributes[:images] = data['images'].split(',') if data['images'].present?
    ...
    0.upto( variants.length - 1 ) do |i|
      variant = Variant.find_by( :product_id => _product.id, :sku => variants[i]['sku'] ) unless variants[i]['sku'].blank?
      variant = Variant.create( :product_id => _product.id, :sku => variants[i]['sku'], :price => variants[i]['price'], :length => variants[i]['length'], :height => variants[i]['height'], :depth => variants[i]['depth'], :state => variants[i]['state']) if variant.blank?
      ...
      variants[i]['options'].keys.each do |key|
        ...
        unless variants[i]['options'][key].nil? 
          ...
          new_option_value = OptionValue.where( :option_id => option.id, :value => variants[i]['options'][key].titleize ).first
          new_option_value = OptionValue.create( :option_id => option.id, :value => variants[i]['options'][key].titleize ) if new_option_value.nil?
  ...
  def get_product_data product
    ...
    data = Hash[product_keys.collect { |v| [v, eval("product.#{v}")] }]
    data['images'] = product.images.map{ |img| img.src }.join(',')
    data
  end
